...
$allowed_sort = ['nama', 'divisi', 'status', 'waktu_mulai', 'durasi'];
...
while($r = mysqli_fetch_assoc($q)){
    ...
    // TIDAK menampilkan tombol reset untuk view karyawan
    echo '<tr id="row-'.$user_id.'"><td>'.htmlspecialchars($r['nama']).'</td><td>'.$divisi.'</td><td>'.$badge.'</td><td>'.($r['waktu_mulai']?htmlspecialchars($r['waktu_mulai']):'-').'</td><td>'.($r['waktu_selesai']?htmlspecialchars($r['waktu_selesai']):'-').'</td><td>'.($r['durasi']?intval($r['durasi']):'-').'</td></tr>';
}
}
echo '</tbody></table>';
echo '</div>';
